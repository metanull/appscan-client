  ...
  try {
    ...
    if (options.json) {
      ...
      executions.forEach((execution, index) => {
        console.log(`${index + 1}. ${chalk.bold('Execution ID:')} ${chalk.gray(execution.Id || 'N/A')}`);
        const statusColor = execution.Status === 'Ready' ? 'green' : execution.Status === 'Failed' ? 'red' : 'yellow';
        console.log(`   ${chalk.dim('Status:')} ${chalk[statusColor](execution.Status || 'N/A')}`);
        if (execution.StartedAt) {
          console.log(`   ${chalk.dim('Started:')} ${new Date(execution.StartedAt).toLocaleString()}`);
        }
