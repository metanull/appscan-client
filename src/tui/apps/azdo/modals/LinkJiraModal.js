 ...
 * @returns {JSX.Element}
 */
export const LinkJiraModal = React.memo(({ alertCount, onLink, onClose }) => {
    ...
    <Modal width={70} height={22}>
      <Panel title="🔗 Link Jira Issue" borderColor="green">
        <Box flexDirection="column" marginTop={1}>
          ...
          <Text dimColor>Store Jira ID in alert comment metadata</Text>
        </Box>

        <Box flexDirection="column" marginTop={1} paddingX={1}>
          <Text color="yellow">⚠️ Important:</Text>
          <Text dimColor>• Active alerts: Set to Dismissed (Unknown)</Text>
          <Text dimColor>
            • Non-Active alerts: Temporarily set to Active, then restored
          </Text>
          <Text dimColor>• Existing comments preserved when possible</Text>
          <Text dimColor>• Jira ID stored as metadata in comment field</Text>
        </Box>
