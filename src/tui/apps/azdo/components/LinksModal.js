...
export const LinksModal = React.memo(
  ({ alert, project, repository, config, azdoService, onClose }) => {
    ...
    // Alert Web URL (primary - in first position)
    ...
    if (alertWebUrl) {
      links.push({
        label: '🔗 Alert Web URL',
        url: alertWebUrl,
        description: 'Open the alert in Azure DevOps browser',
      });
    }

    // Azure DevOps Alert link (from alert object, if different)
    if (alert.url && alert.url !== alertWebUrl) {
      links.push({
