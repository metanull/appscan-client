 ...
 * @param {Object} props.AsocService - Service for URL generation
 * @param {Function} props.onClose - Callback when modal is closed
 * @returns {JSX.Element}
 */
export const LinksModal = React.memo(
  ({ issue, app, scan, config, AsocService, onClose }) => {
    ...
    useEffect(() => {
      if (issue && issue.Id && AsocService) {
        AsocService.getIssueComments(issue.Id)
          .then((commentsList) => {
    ...
    }, [issue?.Id, AsocService]);

    // Fetch focused article URL on mount
    useEffect(() => {
      if (issue.IssueTypeId && AsocService) {
        setLoadingArticleUrl(true);
        AsocService.getFocusedArticleUrl(issue)
          .then((url) => {
            setArticleUrl(url);
          })
          .catch((_error) => {
    ...
    if (app?.Id) {
      links.push({
        ...
        url: AsocService.getApplicationUrl(app.Id),
      });
    }
    ...
    if (app?.Id && scanId) {
      links.push({
        label: '🔗 AppScan Scan',
        url: AsocService.getScanUrl(app.Id, scanId),
      });
    }
    ...
    if (app?.Id && issue.Id) {
      links.push({
        label: '🔗 AppScan Issue',
        url: AsocService.getIssueUrl(app.Id, issue.Id),
      });
    }
    ...
    const jiraUrl = AsocService.getJiraUrl(issue);
    if (jiraUrl) {
      links.push({
