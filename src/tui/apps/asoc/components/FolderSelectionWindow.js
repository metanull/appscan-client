 ...
 * @param {Object} props.AsocService - Service for scan operations
...
export const FolderSelectionWindow = React.memo(
  ({
    ...
    const filteredScans = useMemo(() => {
      ...
      if (hideEmpty) {
        filtered = filtered.filter((scan) => {
          const { total } = AsocService?.getScanIssueCounts(scan) || {
            total: 0,
          };
          return total > 0;
        });
    ...
    }, [scans, debouncedSearchText, filterType, hideEmpty, AsocService]);
    ...
    const renderItem = useCallback(
      (scan, isSelected) => {
        ...
        const { critical, high, medium, low, info, total } =
          AsocService?.getScanIssueCounts(scan) || {
            critical: 0,
            high: 0,
            medium: 0,
            low: 0,
