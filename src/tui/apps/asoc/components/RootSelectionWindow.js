 ...
 * @param {Object} props.AsocService - Service for application/scan operations
  ...
  ({
    ...
    const filteredApps = useMemo(() => {
      ...
      filtered = filtered.filter((app) => {
        const scanCount = AsocService?.getAppScanCount(app) || 0;
        return scanCount > 0;
      });
      ...
      if (hideEmpty) {
        filtered = filtered.filter((app) => {
          const { total } = AsocService?.getAppIssueCounts(app) || {
            inProgress: 0,
            active: 0,
            total: 0,
          };
      ...
      filtered.sort((a, b) => {
        if (sortBy === 'name') {
          ...
          const aCount = AsocService?.getAppIssueCounts(a)?.active || 0;
          const bCount = AsocService?.getAppIssueCounts(b)?.active || 0;
      ...
      return filtered;
    }, [applications, debouncedSearchText, sortBy, hideEmpty, AsocService]);
    ...
    const renderItem = useCallback(
      (app, isSelected) => {
        const { inProgress, active, total } = AsocService?.getAppIssueCounts(
          app
        ) || {
          inProgress: 0,
          active: 0,
