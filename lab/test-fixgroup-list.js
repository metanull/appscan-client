...
async function testFixGroupList() {
  try {
    ...
    console.log(
      `   Total applications available: ${appsResponse.Items.length}\n`
    );
    ...
    try {
      ...
      console.log(
        `   Has TotalCount: ${fixGroupsResponse.TotalCount !== undefined}`
      );
      ...
      if (fixGroupsResponse.Items && fixGroupsResponse.Items.length > 0) {
        ...
        fixGroupsResponse.Items.forEach((group) => {
          ...
          severityCounts[group.Severity] =
            (severityCounts[group.Severity] || 0) + 1;
        });
    ...
    try {
      const correlationResponse =
        await service.api.v4.Apps_GetAppCorrelationGroups(app.Id, {});
      ...
      console.log(
        `   Has TotalCount: ${correlationResponse.TotalCount !== undefined}`
      );
    ...
    try {
      ...
      console.log(
        `✅ Filtered query succeeded, returned ${filteredResponse.Items?.length || 0} groups\n`
      );
    } catch (error) {
    ...
    try {
      ...
      console.log(
        `✅ Select query succeeded, returned ${selectResponse.Items?.length || 0} groups`
      );
