...
async function main() {
  try {
    ...
    console.log(
      `=== Advanced Security Alerts for Project: ${project.name} ===\n`
    );
    ...
    for (const repo of repos) {
      ...
      try {
        ...
        for (const alert of displayAlerts) {
          console.log(
            `    [${alert.alertId || alert.id}] ${alert.title || alert.ruleName || '(no title)'}`
          );
          console.log(
            `      Type: ${alert.alertType || 'N/A'}, Severity: ${alert.severity || 'N/A'}, State: ${alert.state || 'N/A'}`
          );
        }
        console.log();
      } catch (err) {
