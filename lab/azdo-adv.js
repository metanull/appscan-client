...
export function getAdvsecBase() {
  const org =
    ...
    (process.env.AZDO_ORG_URL &&
      process.env.AZDO_ORG_URL.replace('https://dev.azure.com/', ''));
  if (!org) return undefined;
  return `https://advsec.dev.azure.com/${org}`;
}

export async function listRepositoryAlerts({
  projectName,
  repositoryId,
  ...
  if (!base)
    throw new Error(
      'Advsec base could not be determined from env (AZURE_DEVOPS_ORG or AZDO_OR)'
    );
...
export async function listFirstProjectRepoAlerts({ apiVersion } = {}) {
  ...
  const project =
    projects.find((p) => p.name === 'MembersPortal') || projects[0];
  const repos = await listRepositories(project.name);
  const repo = repos[0];
  return listRepositoryAlerts({
