...
async function pageAllAlerts(
  ...
  do {
    ...
    for (const [k, v] of Object.entries(params)) {
      if (v === undefined || v === null) continue;
      if (Array.isArray(v)) for (const it of v) qp.append(k, it);
    ...
    continuation =
      r &&
      r.headers &&
      (r.headers['x-ms-continuationtoken'] ||
        r.headers['X-Ms-ContinuationToken'])
        ? r.headers['x-ms-continuationtoken'] ||
          r.headers['X-Ms-ContinuationToken']
        : null;
...
function extractDependencyNames(alert) {
  ...
  if (Array.isArray(alert.relatedLocations)) {
    for (const rl of alert.relatedLocations)
      if (rl && rl.logicalLocation && rl.logicalLocation.fullyQualifiedName)
        names.add(rl.logicalLocation.fullyQualifiedName);
  }
...
function extractPipelineNames(alert) {
  ...
  if (alert.analysisInfo && alert.analysisInfo.pipeline)
    names.add(alert.analysisInfo.pipeline);
...
function extractPhaseNames(alert) {
  ...
  if (Array.isArray(alert.analysisInfo?.phases)) {
    for (const ph of alert.analysisInfo.phases)
      if (ph && ph.name) p.add(ph.name);
  }
...
function extractToolNames(alert) {
  const t = new Set();
  if (Array.isArray(alert.tools))
    for (const tool of alert.tools) if (tool && tool.name) t.add(tool.name);
  return Array.from(t);
}

function extractValidity(alert) {
  return alert.validityDetails && alert.validityDetails.validityStatus
    ? alert.validityDetails.validityStatus
    : undefined;
}
...
async function run() {
  try {
    ...
    const orgName = deriveOrgNameFromServerUrl(
      conn.serverUrl ||
        process.env.AZDO_ORG_URL ||
        process.env.AZDO_OR ||
        process.env.AZURE_DEVOPS_ORG_URL
    ...
    if (!orgName)
      throw new Error('Cannot determine org name for advsec endpoint');
    ...
    for (const project of targetProjects) {
      ...
      for (const repo of repos) {
        ...
        const sample = await pageAllAlerts(
          conn,
          advBase,
          project.name,
          repo.id,
        ...
        const types = Array.from(
          new Set(sample.map((a) => a.alertType).filter(Boolean))
        );
        ...
        if (types.length > 0) {
          ...
          const d = r && r.result ? r.result : r;
          console.log(
            '   First 3 for alertType=' + v + ':',
            firstN(d && d.value ? d.value : d, 3).map(
              (a) =>
                `${a.alertId || a.id} | ${a.title || a.ruleName || '(no title)'}`
            )
          );
        }
        ...
        const confidences = Array.from(
          new Set(sample.map((a) => a.confidence).filter(Boolean))
        );
        ...
        for (const a of sample) {
          for (const n of extractDependencyNames(a)) deps.add(n);
        }
        const depsArr = Array.from(deps);
        console.log(
          '  Dependency Names (sample):',
          depsArr.slice(0, 10).join(', ')
        );
        if (depsArr.length > 0) {
          const v = depsArr[0];
        ...
        const fdates = Array.from(
          new Set(sample.map((a) => a.firstSeenDate).filter(Boolean))
        ).slice(0, 10);
        console.log(
          '  Distinct firstSeenDate (sample):',
          fdates.slice(0, 6).join(', ')
        );
        ...
        console.log(
          '   First 3 alerts after 2025-06-01:',
          firstN(dfrom && dfrom.value ? dfrom.value : dfrom, 3).map(
            (a) =>
              `${a.alertId || a.id} | ${a.lastSeenDate || a.firstSeenDate || '(no date)'} | ${a.title || a.ruleName || '(no title)'}`
          )
        );
        ...
        for (const a of sample)
          for (const k of extractKeywords(a)) keywords.add(k);
        ...
        if (keys.length > 0) {
          ...
          console.log(
            '   First 3 for keyword=' + k + ':',
            firstN(dk && dk.value ? dk.value : dk, 3).map(
              (a) =>
                `${a.alertId || a.id} | ${a.title || a.ruleName || '(no title)'}`
            )
          );
        ...
        for (const a of sample)
          for (const p of extractPipelineNames(a)) pnames.add(p);
        ...
        if (pArr.length > 0) {
          ...
          console.log(
            '   First 3 for pipelineName=' + v + ':',
            firstN(dp && dp.value ? dp.value : dp, 3).map(
              (a) =>
                `${a.alertId || a.id} | ${a.title || a.ruleName || '(no title)'}`
            )
          );
          ...
          for (const a of dp && dp.value ? dp.value : dp)
            for (const ph of extractPhaseNames(a)) phaseNames.add(ph);
          const phArr = Array.from(phaseNames);
          console.log(
            '   Distinct phaseNames for pipeline:',
            phArr.slice(0, 10).join(', ')
          );
          if (phArr.length > 0) {
            const ph = phArr[0];
            ...
            console.log(
              '    First 3 for phaseName=' + ph + ':',
              firstN(dph && dph.value ? dph.value : dph, 3).map(
                (a) =>
                  `${a.alertId || a.id} | ${a.title || a.ruleName || '(no title)'}`
              )
            );
        ...
        for (const a of sample) {
          const r = extractRuleIdsAndNames(a);
          r.ids.forEach((i) => ruleIds.add(i));
          r.names.forEach((n) => ruleNames.add(n));
        }
        console.log(
          '  RuleIds (sample):',
          Array.from(ruleIds).slice(0, 10).join(', ')
        );
        console.log(
          '  RuleNames (sample):',
          Array.from(ruleNames).slice(0, 10).join(', ')
        );
        if (ruleIds.size > 0) {
        ...
        console.log(
          '   First 3 alerts before 2025-06-01:',
          firstN(dto && dto.value ? dto.value : dto, 3).map(
            (a) =>
              `${a.alertId || a.id} | ${a.lastSeenDate || a.firstSeenDate || '(no date)'} | ${a.title || a.ruleName || '(no title)'}`
          )
        );
        ...
        for (const a of sample)
          for (const t of extractToolNames(a)) toolNames.add(t);
        console.log(
          '  Tool Names (sample):',
          Array.from(toolNames).slice(0, 10).join(', ')
        );
        if (toolNames.size > 0) {
        ...
        // 10. Secrets validity
        const validityVals = new Set();
        for (const a of sample) {
          const v = extractValidity(a);
          if (v) validityVals.add(v);
        }
        console.log(
          '  Validity values (sample):',
          Array.from(validityVals).join(', ')
        );
        ...
        const ids = Array.from(
          new Set(sample.map((a) => a.alertId).filter(Boolean))
        );
        ...
        const modDates = Array.from(
          new Set(
            sample.map((a) => a.lastSeenDate || a.fixedDate).filter(Boolean)
          )
        ).slice(0, 10);
        console.log(
          '  Modified dates (sample):',
          modDates.slice(0, 6).join(', ')
        );
        ...
        console.log(
          '   First 3 modified since 2025-06-01:',
          firstN(dmod && dmod.value ? dmod.value : dmod, 3).map(
            (a) =>
              `${a.alertId || a.id} | ${a.lastSeenDate || a.fixedDate || '(no date)'} | ${a.title || a.ruleName || '(no title)'}`
          )
        );
        ...
        for (const o of orders) {
          ...
          console.log(
            ...
            firstN(d && d.value ? d.value : d, 3).map(
              (a) =>
                `${a.alertId || a.id} | ${o === 'severity' ? a.severity : o === 'firstSeen' ? a.firstSeenDate : o === 'lastSeen' ? a.lastSeenDate : o === 'fixedOn' ? a.fixedDate : a.alertId || a.id} | ${a.title || a.ruleName || '(no title)'}`
            )
          );
        }
        ...
        console.log(
          '   Top 10 orderBy=id:',
          (dtr1 && dtr1.value ? dtr1.value : dtr1).length
        );
        ...
        console.log(
          '   Top 10 orderBy=severity:',
          (dtr2 && dtr2.value ? dtr2.value : dtr2).length
        );
      }
    }
