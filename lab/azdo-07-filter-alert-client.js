  ...
  try {
    ...
    for (const project of targetProjects) {
      ...
      for (const repo of repos) {
        ...
        if (types.length > 0) {
          const v = types[0];
          const d = await alertApi.getAlerts(
            project.name,
            repo.id,
            3,
            undefined,
        ...
        const dfrom = await alertApi.getAlerts(
          project.name,
          repo.id,
          3,
          undefined,
        ...
        if (keys.length > 0) {
          const k = keys[0];
          const dk = await alertApi.getAlerts(
            project.name,
            repo.id,
            3,
            undefined,
        ...
        if (pArr.length > 0) {
          const v = pArr[0];
          const dp = await alertApi.getAlerts(
            project.name,
            repo.id,
            3,
            undefined,
          ...
          if (phArr.length > 0) {
            const ph = phArr[0];
            const dph = await alertApi.getAlerts(
              project.name,
              repo.id,
              3,
              undefined,
              { pipelineName: v, phaseName: ph }
            );
            console.log(
        ...
        const dto = await alertApi.getAlerts(
          project.name,
          repo.id,
          3,
          undefined,
        ...
        const dmod = await alertApi.getAlerts(
          project.name,
          repo.id,
          3,
          undefined,
          { modifiedSince: modifiedSinceFilter }
        );
        console.log(
        ...
        const tr2 = await alertApi.getAlerts(
          project.name,
          repo.id,
          10,
          'severity'
