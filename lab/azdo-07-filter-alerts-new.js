...
function extractKeywords(alert) {
  ...
  const items = alert.title.split(/[\W_]+/)
    .map(s => s.trim())
    .filter(s => s.length > 3);
...
async function main() {
  try {
    ...
    async function fetchAllAlertsUsingApi(alertApi, projectName, repositoryId, criteria = {}, pageTop = Number(process.env.AZDO_ALERTS_PAGE_TOP) || 100) {
      const all = [];
    ...
    for (const repo of repos) {
      ...
      try {
        ...
        const allAlerts = await fetchAllAlertsUsingApi(alertApi, project.name, repo.id);
        ...
        const types = Array.from(new Set(allAlerts.map(a => a.alertType).filter(Boolean)));
        if (types.length > 0) {
          ...
          const typeAlerts = await fetchAllAlertsUsingApi(alertApi, project.name, repo.id, { alertType: types[0] });
          console.log(`    Results for type=${types[0]}: ${typeAlerts?.length || 0} alert(s)`);
          if (typeAlerts && typeAlerts.length > 0) {
            console.log(`      Example: ${typeAlerts[0].title || typeAlerts[0].ruleName || '(no title)'}`);
          }
          console.log();
        }

        // 2. Filter by Severity
        const severities = Array.from(new Set(allAlerts.map(a => a.severity).filter(Boolean)));
        if (severities.length > 0) {
          console.log('  ⚠️  Order by Severity:');
          const sevAlerts = await alertApi.getAlerts(project.name, repo.id, 5, 'severity');
          console.log(`    Top 5 by severity: ${sevAlerts?.length || 0} alert(s)`);
          if (sevAlerts && sevAlerts.length > 0) {
            for (const alert of sevAlerts) {
              console.log(`      [Severity ${alert.severity}] ${alert.title || alert.ruleName || '(no title)'}`);
            }
          }
          console.log();
        }
        ...
        allAlerts.forEach(a => extractKeywords(a).forEach(k => allKeywords.add(k)));
        ...
        if (keywords.length > 0) {
          ...
          const keywordAlerts = await fetchAllAlertsUsingApi(alertApi, project.name, repo.id, { keywords: keywords[0] });
          console.log(`    Results for keyword="${keywords[0]}": ${keywordAlerts?.length || 0} alert(s)`);
          console.log();
        }
        ...
        const dateAlerts = await fetchAllAlertsUsingApi(alertApi, project.name, repo.id, { fromDate });
        console.log(`    Alerts since ${fromDate}: ${dateAlerts?.length || 0} alert(s)`);
        console.log();
        ...
        for (const order of orderOptions) {
          const ordered = await alertApi.getAlerts(project.name, repo.id, 3, order);
          console.log(`    orderBy="${order}": ${ordered?.length || 0} alert(s)`);
          if (ordered && ordered.length > 0) {
            const first = ordered[0];
            const value = order === 'severity' ? first.severity :
                         order === 'firstSeen' ? first.firstSeenDate :
                         order === 'lastSeen' ? first.lastSeenDate :
        ...
        // 6. Alert ID Filter
        const alertIds = allAlerts.slice(0, 3).map(a => a.alertId).filter(Boolean);
        if (alertIds.length > 0) {
          console.log('  🆔 Filter by Alert IDs:');
          const idAlerts = await fetchAllAlertsUsingApi(alertApi, project.name, repo.id, { alertIds });
          console.log(`    Requested ${alertIds.length} specific alert(s): ${idAlerts?.length || 0} found`);
          console.log();
        }

        console.log('  ✅ Filtering demonstration complete for this repository\n');
