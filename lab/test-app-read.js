...
async function testApplicationRead() {
  try {
    ...
    console.log(
      `📝 Test 2: Test different methods to get single application details`
    );
    console.log(`   Target app: "${testApp.Name}" (${testApp.Id})\n`);

    // Method A: Try the current getApplicationDetails implementation
    console.log(
      'Method A: Using getApplicationDetails() [Apps_Get({ Id: appId })]'
    );
    try {
      ...
      console.log(
        '  Is single object:',
        !appDetails.Items && typeof appDetails === 'object'
      );
      if (appDetails.Items) {
        ...
        if (appDetails.Items.length > 0) {
          ...
          console.log(
            `     Name: ${app.Name}, CustomFields: ${app.CustomFields?.length || 0}`
          );
        }
      } else if (appDetails.Id) {
        console.log('  ✅ Got application data as single object');
        console.log(
          `     Name: ${appDetails.Name}, CustomFields: ${appDetails.CustomFields?.length || 0}`
        );
      } else {
    ...
    try {
      ...
      if (filtered.Items && filtered.Items.length > 0) {
        ...
        console.log(
          `     CustomFields: ${app.CustomFields?.length || 0} fields`
        );
        console.log(
          `     All standard fields present: ${!!(app.Id && app.Name && app.RiskRating)}`
        );
      } else {
    ...
    try {
      ...
      const response = await service.api.rawRequest(
        'GET',
        `/api/v4/Apps/${testApp.Id}`
      );
      ...
      console.log(
        `  ❌ Direct endpoint doesn't exist or failed: ${error.message}`
      );
    }
    console.log('');
    ...
    console.log(
      `  Development Contact: ${appDetails.DevelopmentContact || 'N/A'}`
    );
    console.log(`  Business Owner: ${appDetails.BusinessOwner || 'N/A'}`);
    console.log(`  Tester: ${appDetails.Tester || 'N/A'}`);
    console.log(
      `  Confidentiality Requirement: ${appDetails.ConfidentialityRequirement || 'N/A'}`
    );
    console.log(
      `  Integrity Requirement: ${appDetails.IntegrityRequirement || 'N/A'}`
    );
    console.log(
      `  Availability Requirement: ${appDetails.AvailabilityRequirement || 'N/A'}`
    );
    ...
    if (appDetails.CustomFields && appDetails.CustomFields.length > 0) {
      console.log(
        `✅ Found ${appDetails.CustomFields.length} custom field(s):\n`
      );
      ...
      expectedFields.forEach((fieldName) => {
        const field = appDetails.CustomFields.find(
    ...
    const listHasCustomFields =
      listApp && listApp.CustomFields && listApp.CustomFields.length > 0;
    const detailsHasCustomFields =
      appDetails.CustomFields && appDetails.CustomFields.length > 0;

    if (listHasCustomFields && detailsHasCustomFields) {
      console.log(
        '✅ CustomFields are included in both list and detail responses'
      );
      console.log(`   List response: ${listApp.CustomFields.length} fields`);
      console.log(
        `   Detail response: ${appDetails.CustomFields.length} fields`
      );
    } else if (!listHasCustomFields && detailsHasCustomFields) {
      console.log(
        '⚠️  CustomFields only in detail response, not in list response'
      );
    } else if (listHasCustomFields && !detailsHasCustomFields) {
      console.log(
        '⚠️  CustomFields only in list response, not in detail response'
      );
    } else {
    ...
    try {
      ...
      if (app) {
        console.log(
          `   CustomFields expanded: ${app.CustomFields ? 'Yes' : 'No'}`
        );
        if (app.CustomFields) {
    ...
    console.log(
      '📝 Test 6: Test CustomFields transformation to key-value pairs'
    );
    ...
    if (transformApp && transformApp.CustomFields) {
      ...
      const emptyFields = Object.entries(customFieldsMap).filter(
        ([k, v]) => v === null
      );
      const emptyStringFields = Object.entries(customFieldsMap).filter(
        ([k, v]) => v === ''
      );
      console.log(`  Fields with null value: ${emptyFields.length}`);
      console.log(
        `  Fields with empty string: ${emptyStringFields.length} ${emptyStringFields.length > 0 ? '❌ SHOULD BE NULL!' : '✅'}`
      );
      console.log('');
      ...
      console.log(
        `  customFields.DevOpsProject === null: ${customFieldsMap.DevOpsProject === null}`
      );
      console.log(
        `  customFields.JiraProject || 'default': "${customFieldsMap.JiraProject || 'default'}"`
      );
      console.log(
        `  customFields.JiraParentEpic ?? 'fallback': "${customFieldsMap.JiraParentEpic ?? 'fallback'}"`
      );
      console.log('');
      ...
      console.log(
        `  ✅ Field names: All ${transformApp.CustomFields.length} preserved`
      );
      ...
      console.log(
        '⚠️  IMPORTANT: For UPDATES, we may still need the field IDs!'
      );
      console.log(
        '   If updating custom fields requires the ID, we should keep:'
      );
      console.log(
        '   - Simplified structure for READ operations (commands, reports)'
      );
    ...
    console.log(
      '   - CustomFields can be safely transformed to key-value pairs for READ'
    );
