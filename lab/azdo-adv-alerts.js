...
async function main() {
  ...
  if (r.status === 200) {
    console.log('alerts count:', Array.isArray(r.data?.value) ? r.data.value.length : 'unknown');
    console.log('sample:', JSON.stringify(Array.isArray(r.data?.value) ? r.data.value.slice(0,5) : r.data, null, 2));
  } else {
