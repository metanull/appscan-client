...
async function main() {
  try {
    ...
    if (!targetAlert || !targetRepo) {
      ...
      console.log(
        '   Advanced Security may not be enabled or there are no alerts.'
      );
      process.exit(1);
    }
    ...
    console.log('\n' + '='.repeat(70));
    console.log(
      'APPROACH 1: Storing Custom Metadata in Alert Comments (RECOMMENDED)'
    );
    ...
    console.log(
      '   Benefits: Works immediately, no Azure DevOps work items required'
    );
    ...
    const customMetadata = {
      ...
      userComment:
        'Investigating SQL credential leak. Linked to Jira ticket for tracking remediation.',
    };
    ...
    await sleep(1000);
    updatedAlert = await alertApi.getAlert(
      project.name,
      targetAlert.alertId,
      targetRepo.id
    );
    ...
    if (updatedAlert.dismissal?.message) {
      try {
        ...
        console.log(
          `   External System: ${parsedData._metadata.externalSystem}`
        );
    ...
    console.log(
      '   It does NOT support custom metadata types (Jira, tracking IDs, etc.)'
    );
    ...
    const testCases = [
      {
        rel: 'jira',
        url: 'https://jira.example.com/browse/SEC-12345',
        description: 'Jira URL',
      },
      {
        rel: 'tracking-id',
        url: 'https://tracker.example.com/TRACK-12345',
    ...
    for (const testCase of testCases) {
      try {
        console.log(
          `   Testing rel="${testCase.rel}" (${testCase.description})...`
        );
    ...
    await sleep(1000);
    const restoredAlert = await alertApi.getAlert(
      project.name,
      targetAlert.alertId,
      targetRepo.id
    );
    ...
    console.log(
      '   • Supports any custom fields: tracking IDs, Jira URLs, labels, etc.'
    );
    ...
    console.log(
      '   • Only supports rel="workitem" with Azure DevOps work items'
    );
    console.log(
      '   • Custom rel types (jira, tracking-id, external, etc.) are rejected'
    );
    console.log('   • Not suitable for external system integration\n');

    console.log('💡 Implementation Recommendations:');
    console.log(
      '   1. Use updateAlert() with JSON in dismissedComment for custom metadata'
    );
    console.log(
      '   2. Establish a standard metadata schema for your organization'
    );
    console.log('   3. Use _metadata prefix to distinguish from user comments');
    console.log(
      '   4. Include both metadata and human-readable comment in same field'
    );
    console.log(
      '   5. Create helper functions to serialize/deserialize metadata\n'
    );
    ...
    console.log(
      '   5. When fixed → Update state to Fixed, metadata persists\n'
    );

    console.log('✅ Script completed successfully!');
    console.log(
      `   Alert URL: ${orgUrl}/${project.name}/_git/${targetRepo.name}/alerts/${targetAlert.alertId}`
    );
    console.log(
      '   Custom metadata has been demonstrated using alert comments.\n'
    );
