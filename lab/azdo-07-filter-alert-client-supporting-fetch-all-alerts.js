...
const DEFAULT_PROJECTS = (process.env.AZDO_PROJECTS && process.env.AZDO_PROJECTS.split(',').map(s=>s.trim()).filter(Boolean)) || ['Phoenix','Agora'];
...
async function fetchAllAlertsUsingApi(alertApi, projectName, repositoryId, criteria = {}, pageTop = PAGE_TOP) {
  ...
  do {
    ...
    if (pageCount > 10000) throw new Error('Too many pages while paginating alerts, aborting');
...
async function main() {
  try {
    ...
    const targetProjects = projects.filter(p => DEFAULT_PROJECTS.map(x=>x.toLowerCase()).includes(p.name.toLowerCase()));
    ...
    for (const project of targetProjects) {
      ...
      for (const repo of repos) {
        try {
          ...
          console.error(`  Error fetching alerts for repo ${repo.name}:`, e && e.message ? e.message : String(e));
        }
      }
