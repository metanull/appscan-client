  ...
  try {
    ...
    try {
      ...
      if (verifyApp.Description === testDescription) {
        ...
        console.log(`   ❌ Mismatch! Expected "${testDescription}", got "${verifyApp.Description}"`);
      }
    ...
    try {
      ...
      const jiraField = originalApp._customFieldsRaw.find(f => f.Name === 'JiraProject');
      if (!jiraField) {
      ...
      if (verifyApp2.customFields.JiraProject === testJiraProject) {
        ...
        console.log(`   ❌ Mismatch! Expected "${testJiraProject}", got "${verifyApp2.customFields.JiraProject}"`);
      }
    ...
    try {
      // Find the custom field ID
      const devOpsField = originalApp._customFieldsRaw.find(f => f.Name === 'DevOpsProject');
      if (!devOpsField) {
      ...
      const devOpsMatch = verifyApp3.customFields.DevOpsProject === testDevOpsProject;
      
      console.log(`   ${typeMatch ? '✅' : '❌'} Type: ${verifyApp3.Type}`);
      console.log(`   ${devOpsMatch ? '✅' : '❌'} DevOpsProject: ${verifyApp3.customFields.DevOpsProject}`);
      ...
      const devOpsRestored = finalApp3.customFields.DevOpsProject === originalDevOpsProject;
