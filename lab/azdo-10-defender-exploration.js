 ...
 */
async function callDefenderAPI(
  credential,
  subscriptionId,
  path,
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `API call failed: ${response.status} ${response.statusText} - ${error}`
    );
  }
  ...
  try {
        ...
        if (connectors.length > 0) {
          ...
          connectors.forEach((conn, i) => {
            ...
            if (conn.properties?.offerings) {
              console.log(
                `     Offerings: ${conn.properties.offerings.map((o) => o.offeringType).join(', ')}`
              );
            }
          });
        } else {
          ...
          console.log(
            '   You need to set up a DevOps security connector in Azure Portal:'
          );
          console.log(
            '   https://portal.azure.com -> Defender for Cloud -> Environment settings'
          );
        }
      }
    } catch (error) {
    ...
    try {
      ...
      if (!resourceGroup || !connectorName) {
        console.log(
          '⚠️  Skipped: Set AZURE_RESOURCE_GROUP and AZURE_SECURITY_CONNECTOR to test'
        );
      } else {
        ...
        if (orgs.length > 0) {
          ...
          orgs.forEach((org, i) => {
            ...
            console.log(
              `     Onboarding State: ${org.properties?.onboardingState}`
            );
            console.log(
              `     Provisioning State: ${org.properties?.provisioningState}`
            );
          });
        }
      }
    ...
    try {
      ...
      if (!resourceGroup || !connectorName || !orgName) {
        console.log(
          '⚠️  Skipped: Set AZURE_RESOURCE_GROUP, AZURE_SECURITY_CONNECTOR, and AZURE_DEVOPS_ORG'
        );
      } else {
        ...
        if (projects.length > 0) {
          ...
          projects.slice(0, 3).forEach((proj, i) => {
            ...
            console.log(
              `     Onboarding State: ${proj.properties?.onboardingState}`
            );
          });
        }
      }
    ...
    try {
      ...
      if (!resourceGroup || !connectorName || !orgName) {
        ...
        console.log(
          `Found ${repos.length} repositories in project ${projectName}`
        );
        ...
        if (repos.length > 0) {
          ...
          repos.forEach((repo, i) => {
            ...
            console.log(
              `     Onboarding State: ${repo.properties?.onboardingState}`
            );
            if (repo.properties?.actionableRemediation) {
              console.log(
                `     Actionable Remediation: ${repo.properties.actionableRemediation.state}`
              );
            }
          });
        }
    ...
    try {
      ...
      if (alerts.length > 0) {
        ...
        alerts.slice(0, 3).forEach((a, i) => {
          console.log(
            `  ${i + 1}. ${a.properties?.alertDisplayName || a.name}`
          );
          console.log(
            `     Severity: ${a.properties?.severity}, Status: ${a.properties?.status}`
          );
    ...
    try {
      ...
      if (secureScores.length > 0) {
        ...
        secureScores.forEach((s, i) => {
          console.log(
            `  ${i + 1}. ${s.name}: Current: ${s.properties?.score?.current}, Max: ${s.properties?.score?.max}`
          );
        });
      }
    } catch (error) {
    ...
    console.log(
      '  1. List Azure DevOps organizations (via security connector)'
    );
    ...
    console.log(
      '  - Alert filtering may have different options than native API'
    );
    ...
    console.log(
      '  1. Granular alert filtering (by pipeline, phase, rule, etc.)'
    );
    ...
    console.error(
      '  - AZURE_RESOURCE_GROUP: Resource group with security connector'
    );
