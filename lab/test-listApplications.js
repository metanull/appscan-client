...
async function testListApplications() {
  try {
    ...
    console.log(
      `   Has other properties: ${Object.keys(response).filter((k) => k !== 'Items').length > 0 ? 'Yes' : 'No'}`
    );
    console.log('');
    ...
    console.log(
      `   CustomFields removed: ${firstApp.CustomFields === undefined ? '✅' : '❌'}`
    );
    console.log('');
    ...
    if (firstApp.customFields) {
      ...
      console.log(
        `   Is object: ${typeof firstApp.customFields === 'object' && !Array.isArray(firstApp.customFields) ? '✅' : '❌'}`
      );
      ...
      Object.entries(firstApp.customFields).forEach(([key, value]) => {
        ...
        console.log(
          `     ${key}: ${value === null ? 'null' : `"${value}"`} (${valueType})`
        );
      });
      console.log('');

      // Verify no empty strings
      const emptyStrings = Object.entries(firstApp.customFields).filter(
        ([k, v]) => v === ''
      );
      console.log(
        `   Empty strings found: ${emptyStrings.length} ${emptyStrings.length === 0 ? '✅' : '❌'}`
      );
    } else {
    ...
    if (firstApp._customFieldsRaw) {
      ...
      console.log(
        `   Type: ${Array.isArray(firstApp._customFieldsRaw) ? 'array' : typeof firstApp._customFieldsRaw} ${Array.isArray(firstApp._customFieldsRaw) ? '✅' : '❌'}`
      );
    ...
    console.log(
      '📝 Step 6: Find and verify Agora app with real custom field data'
    );
    ...
    if (!agoraApp) {
      ...
      throw new Error(
        'Agora app (71d969b5-8d40-4921-a3ce-8de07c04da7c) not found - cannot validate with real data'
      );
    }
    ...
    const expectedFields = {
      ...
      JiraProject: 'AGR',
      DevOpsRepo:
        'Agora,agora-config,Agora-db,agora-event-grid-poc,agora-event-grid-poc-sb,Backup',
    ...
    Object.entries(expectedFields).forEach(([key, expectedValue]) => {
      ...
      if (!matches) {
        console.log(
          `     ${icon} ${key}: Expected "${expectedValue}", got "${actualValue || '(not set)'}"`
        );
        allMatch = false;
      } else {
    ...
    response.Items.forEach((app, index) => {
      if (app.CustomFields !== undefined) {
        console.log(
          `   ❌ App ${index + 1} (${app.Name}) still has CustomFields property`
        );
        inconsistencies++;
      }
      if (app.customFields && typeof app.customFields !== 'object') {
        console.log(
          `   ❌ App ${index + 1} (${app.Name}) has invalid customFields type`
        );
        inconsistencies++;
      }
      if (app.customFields && Array.isArray(app.customFields)) {
        console.log(
          `   ❌ App ${index + 1} (${app.Name}) has customFields as array (should be object)`
        );
        inconsistencies++;
      }
    });
    ...
    if (inconsistencies === 0) {
      console.log(
        `   ✅ All ${response.Items.length} applications have consistent structure`
      );
    } else {
    ...
    console.log(
      '📝 Step 8: Verify backward compatibility (existing properties preserved)'
    );
    const requiredProps = [
      'Id',
      'Name',
    ...
    console.log('📊 Summary:');
    console.log(
      `   ✅ listApplications() returns Items array (${response.Items.length} apps)`
    );
    console.log(
      '   ✅ CustomFields transformed to customFields object for all apps'
    );
