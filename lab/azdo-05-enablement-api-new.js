...
function short(v) {
  try {
    ...
    if (typeof v === 'object')
      return JSON.stringify(v, Object.keys(v).slice(0, 10), 2);
    return String(v);
  } catch (e) {
...
async function tryCall(api, name, ...args) {
  if (typeof api[name] !== 'function')
    throw new Error(`Method ${name} not a function`);
  try {
    return await api[name](...args);
  } catch (err) {
    // Re-throw in a richer error
    const e = new Error(
      `Invocation of ${name} failed: ${err?.message || String(err)}`
    );
    e.cause = err;
    throw e;
  }
...
async function main() {
  try {
    ...
    const mgmtFactoryNames = Object.getOwnPropertyNames(
      Object.getPrototypeOf(conn)
    ).filter(
      (n) =>
        n.toLowerCase().includes('management') ||
        n.toLowerCase().includes('managementapi') ||
        n.toLowerCase().includes('get')
    ); // keep broad

    // Prefer known-getManagementApi
    const mgmtFactory =
      typeof conn.getManagementApi === 'function'
        ? 'getManagementApi'
        : pickBest(mgmtFactoryNames, /management/i);

    if (!mgmtFactory || typeof conn[mgmtFactory] !== 'function') {
      console.error(
        '❌ Management API factory not available on this WebApi instance. Cannot use SDK enablement APIs.'
      );
      process.exit(1);
    }
    ...
    const mgmtProto = Object.getOwnPropertyNames(
      Object.getPrototypeOf(mgmt)
    ).filter((n) => typeof mgmt[n] === 'function');
    const enableMethods = mgmtProto.filter((n) => /enable/i.test(n));

    if (!enableMethods || enableMethods.length === 0) {
      console.error(
        '❌ No enablement-related methods were discovered on the management API instance.'
      );
      console.error(
        'Available methods (sample):',
        mgmtProto.slice(0, 40).join(', ')
      );
      process.exit(1);
    }
    ...
    const project =
      projects.find((p) => p.name === targetProjectName) || projects[0];
    ...
    let orgResult;
    const orgCandidates = enableMethods.filter((n) =>
      /org|organization|organizationenablement|getenablement|enablement$/i.test(
        n
      )
    );
    // fallback to generic getEnablement
    if (orgCandidates.length === 0)
      orgCandidates.push(...enableMethods.filter((n) => /enablement/i.test(n)));
      ...
      try {
        ...
        for (const args of tryArgs) {
          try {
            ...
            orgResult = res;
            console.log(
              `Called ${candidate}(${args.map((a) => short(a)).join(', ')})`
            );
            console.log(
              'Org enablement result keys:',
              Object.keys(orgResult || {})
                .slice(0, 20)
                .join(', ')
            );
            invoked = true;
    ...
    if (!invoked) {
      console.log(
        'Could not invoke a management API method for organization-level enablement.'
      );
    }
    ...
    const projCandidates =
      enableMethods.filter((n) => /project/i.test(n)) || [];
    invoked = false;
    for (const candidate of projCandidates.length
      ? projCandidates
      : enableMethods) {
      try {
        ...
        for (const args of tryArgs) {
          try {
            ...
            console.log(
              'Project enablement result keys:',
              Object.keys(projResult || {})
                .slice(0, 20)
                .join(', ')
            );
            invoked = true;
    ...
    if (!invoked) {
      console.log(
        'Could not invoke a management API method for project-level enablement.'
      );
    }
    ...
    const repoCandidates =
      enableMethods.filter((n) => /repo|repository/i.test(n)) || [];
    invoked = false;
    ...
    function findRepoEntry(list, repoId) {
      ...
      for (const e of list) {
        ...
        const candidates = [
          ...
          e.id,
          e.repository?.id,
          e.repository?.repositoryId,
          e.repository?.repoId,
          e.repository?.name,
          e.repository?.remoteUrl,
        ];
        for (const c of candidates) {
          if (!c) continue;
        ...
        if (
          e.repository &&
          e.repository.name &&
          String(e.repository.name).toLowerCase() ===
            (repo.name || '').toLowerCase()
        )
          return e;
        if (
          e.name &&
          String(e.name).toLowerCase() === (repo.name || '').toLowerCase()
        )
          return e;
      }
      return undefined;
    ...
    function interpretEnablementObject(obj) {
      ...
      for (const k of statusKeys) {
        if (typeof obj[k] === 'string')
          return {
            state: obj[k].toLowerCase().includes('enable')
              ? 'enabled'
              : 'disabled',
            reason: `${k}=${obj[k]}`,
          };
      ...
      if (Object.keys(obj).length === 0)
        return { state: 'unknown', reason: '(empty object)' };

      // Fallback: we couldn't determine
      return {
        state: 'unknown',
        reason: `keys: ${Object.keys(obj).slice(0, 10).join(',')}`,
      };
    }
    ...
    const repoTryArgs = [
      [repo.id],
      [project.id, repo.id],
      [project.name, repo.id],
      [project.name, repo.name, repo.id],
      [repo.name],
      [project.id, repo.name],
    ];

    for (const candidate of repoCandidates.length
      ? repoCandidates
      : enableMethods) {
      for (const args of repoTryArgs) {
        ...
        try {
          try {
            ...
            console.log(
              'Repository enablement result keys:',
              Object.keys(repoResult || {})
                .slice(0, 20)
                .join(', ')
            );
            invoked = true;
    ...
    if (!invoked) {
      console.log(
        'Could not invoke a management API method for repository-level enablement. Will attempt to locate repo records in org/project results.'
      );
    }
    ...
    let matchedEntry;
    matchedEntry =
      matchedEntry || findRepoEntry(orgResult?.reposEnablementStatus, repo.id);
    matchedEntry =
      matchedEntry || findRepoEntry(projResult?.reposEnablementStatus, repo.id);

    if (matchedEntry) {
      console.log(
        'Found repository enablement entry in org/project enablement lists:'
      );
      console.log(short(matchedEntry));
      // interpret
    } else if (repoResult && Object.keys(repoResult).length === 0) {
      console.log(
        'Repository-level API returned an empty object for this repo.'
      );
    }
    ...
    function resolveStatus({ org, proj, repoEntry, repoDirect }) {
      ...
      if (proj) {
        ...
        if (match)
          return {
            which: 'project.reposEnablementStatus',
            result: interpretEnablementObject(match),
          };
      ...
      if (org) {
        ...
        if (match)
          return {
            which: 'org.reposEnablementStatus',
            result: interpretEnablementObject(match),
          };
      ...
      return {
        which: 'unknown',
        result: { state: 'unknown', reason: '(no data anywhere)' },
      };
    }
    ...
    const resolved = resolveStatus({
      org: orgResult,
      proj: projResult,
      repoEntry: matchedEntry,
      repoDirect: repoResult,
    });
    ...
    if (resolved.result && resolved.result.state) {
      ...
      console.log(
        `  state: ${resolved.result.state} - ${resolved.result.reason}`
      );
    } else {
      console.log('  (no data)');
    }
    ...
    if (triedCalls.length > 0) {
      ...
      for (const t of triedCalls.slice(0, 6)) {
        console.log(
          `  ${t.candidate}(${t.args.map((a) => short(a)).join(', ')}): ${t.error}`
        );
      }
      if (triedCalls.length > 6)
        console.log(`  ...and ${triedCalls.length - 6} more`);
    }
