...
async function testApplicationWrite() {
  try {
    ...
    console.log(
      '⚠️  WARNING: This test will modify an application and then restore it.\n'
    );
    ...
    console.log(
      '📝 Step 1: Get current application details (using fixed method)'
    );
    ...
    try {
      ...
      if (verifyApp1.Description !== testDescription) {
        console.log(
          `  ⚠️  Description mismatch: expected "${testDescription}", got "${verifyApp1.Description}"`
        );
        allMatch = false;
      } else {
      ...
      if (verifyApp1.Type !== testType) {
        console.log(
          `  ⚠️  Type mismatch: expected "${testType}", got "${verifyApp1.Type}"`
        );
        allMatch = false;
      } else {
      ...
      if (verifyApp1.Tester !== testTester) {
        console.log(
          `  ⚠️  Tester mismatch: expected "${testTester}", got "${verifyApp1.Tester}"`
        );
        allMatch = false;
      } else {
      ...
      if (devOpsField) {
        ...
        try {
          ...
          if (verifyApp2.customFields.DevOpsProject === testCustomFieldValue) {
            ...
            console.log(
              `  Verified value: ${verifyApp2.customFields.DevOpsProject}\n`
            );
          } else {
            console.log(
              '  ⚠️  Custom field update accepted but value did not change'
            );
    ...
    if (
      ...
      const jiraField = originalApp._customFieldsRaw.find(
        (cf) => cf.Name === 'JiraProject'
      );
      const epicField = originalApp._customFieldsRaw.find(
        (cf) => cf.Name === 'JiraParentEpic'
      );
      ...
      if (jiraField && epicField) {
        ...
        try {
          ...
          const jiraMatch =
            verifyApp3.customFields.JiraProject === testJiraValue;
          const epicMatch =
            verifyApp3.customFields.JiraParentEpic === testEpicValue;

          console.log(
            `  JiraProject: ${jiraMatch ? '✅' : '⚠️'} ${verifyApp3.customFields.JiraProject}`
          );
          console.log(
            `  JiraParentEpic: ${epicMatch ? '✅' : '⚠️'} ${verifyApp3.customFields.JiraParentEpic}\n`
          );
    ...
    if (
      ...
      const confField = originalApp._customFieldsRaw.find(
        (cf) => cf.Name === 'ConfluenceSpace'
      );
      ...
      if (confField) {
        try {
          ...
          console.log(
            `  Description updated: ${verifyApp4.Description?.includes('COMBINED-TEST') ? '✅' : '⚠️'}`
          );
          console.log(
            `  ConfluenceSpace updated: ${verifyApp4.customFields.ConfluenceSpace?.includes('CONF-') ? '✅' : '⚠️'}\n`
          );
        } catch (error) {
    ...
    try {
      ...
      if (
        ...
        restoreData.AppCustomFields = originalApp._customFieldsRaw.map(
          (cf) => ({
            Id: cf.Id,
      ...
      console.log(
        `  Description: ${finalApp.Description === originalApp.Description ? '✅' : '⚠️'} restored`
      );
      console.log(
        `  Type: ${finalApp.Type === originalApp.Type ? '✅' : '⚠️'} restored`
      );
      console.log(
        `  Tester: ${finalApp.Tester === originalApp.Tester ? '✅' : '⚠️'} restored`
      );

      if (originalApp.customFields) {
        console.log('  Custom Fields:');
        Object.entries(originalApp.customFields).forEach(
          ([key, originalValue]) => {
            const currentValue = finalApp.customFields[key];
            const match = currentValue === originalValue;
            console.log(
              `    ${key}: ${match ? '✅' : '⚠️'} ${match ? 'restored' : `${originalValue} → ${currentValue}`}`
            );
          }
        );
      }
      ...
      if (originalApp.customFields) {
        console.log(
          '   Custom Fields:',
          JSON.stringify(originalApp.customFields, null, 2)
        );
      }
      console.log('');
    }
    ...
    console.log('📊 Summary:');
    console.log(
      '   ✅ Standard fields (Description, Type, Tester) can be updated'
    );
    console.log(
      '   ✅ Custom fields can be updated using AppCustomFields array'
    );
    console.log(
      '   ✅ Custom field updates require the field ID from _customFieldsRaw'
    );
