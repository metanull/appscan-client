...
describe('appscan-urls', () => {
  it('generates application/scan/issue urls', () => {
    ...
    expect(getScanUrl('https://x', 'A', 'S')).toBe(
      'https://x/main/myapps/A/scans/S'
    );
    expect(getIssueUrl('https://x', 'A', 'I')).toBe(
      'https://x/main/myapps/A/issues/I'
    );
  });
  ...
  it('builds jira and azure urls', () => {
    ...
    expect(getJiraProjectUrl('https://jira', 'PRJ')).toContain(
      '/projects/PRJ/summary'
    );
    expect(
      getAzureDevOpsProjectUrl('https://dev.azure.com', 'org', 'proj')
    ).toBe('https://dev.azure.com/org/proj');
    expect(
      getAzureDevOpsRepoUrl('https://dev.azure.com', 'org', 'proj', 'repo')
    ).toBe('https://dev.azure.com/org/proj/_git/repo');
    expect(getConfluenceSpaceUrl('https://con', 'ABC')).toContain(
      '/spaces/ABC/overview'
    );
  });
});
